...
function base(opts = {}) {
  ...
  pageNo += 1;
  if (!opts.noNumber) {
    s.addText(String(pageNo), { x: W - M - 0.6, y: H - 0.45, w: 0.6, h: 0.3, fontFace: FONT, fontSize: 10, color: MUTED, align: "right", margin: 0, isTextBox: true });
    s.addText("ETT 油温予測 PoC", { x: M, y: H - 0.45, w: 4, h: 0.3, fontFace: FONT, fontSize: 9, color: MUTED, margin: 0, isTextBox: true });
  }
  return s;
}
...
// ============================================================ 表紙
{
  const s = base({ dark: true, noNumber: true });
  ...
  s.addText("対象: 変圧器 2 台（設備 1 = ETTh1、設備 2 = ETTh2）・2016-07〜2018-06・1 時間ごとの負荷 6 種と油温 OT ／ 主結果はテスト期間 2017-11-01〜2018-06-26（約 8 か月）、予測起点までの観測だけを使う条件で算出", { x: M, y: 6.1, w: W - 2 * M, h: 0.7, fontFace: FONT, fontSize: 10.5, color: "9AA6D6", margin: 0, isTextBox: true });
}
...
// ============================================================ 1 結論
{
  ...
  table(s, [
    ...
    [{ text: "設備 2（ETTh2）", bold: true }, { text: "1.95 ℃", bold: true }, "−18%（リッジ回帰 2.37 ℃）／持続予測比 −56%（4.43 ℃）", "高温 13 区間中 10 区間に事前の的中予測（77%）。通知 12 件・誤通知 3 件（約 8 か月）。検知した区間の先行時間は平均 4.0 時間"],
...
// ============================================================ 5 データ
{
  ...
  bullets(s, ["季節（外気温の代理）と日内周期が支配的 → 対象時刻の時刻・季節を特徴量に入れる価値を検証する", "設備 2 は 6 時間先が最も難しい（持続予測の MAE 4.4 ℃。24 時間先の 3.2 ℃より大きい）", "相関だけでは負荷の追加価値を判断できない → 9 枚目のアブレーションで測る（相関図は付録 B）"], M + 6.5, 4.85, W - 2 * M - 6.5, 1.7, { size: 11.5, gap: 4 });
...
// ============================================================ 6 評価の設計
{
  ...
  const segs = [["学習 2016-07-01 〜 2017-06-30（12 か月）", 12, NAVY], ["検証 2017-07 〜 10（4 か月）", 4, GREEN], ["テスト 2017-11-01 〜 2018-06-26（約 8 か月・5,702 時点）", 8, ORANGE]];
  let x = M;
  ...
  bullets(s, [{ text: "行の切り方", head: true }, "学習に使う行: 対象時刻 T が学習終了前（ラベルが検証期間に食い込まない）", "検証・テストの評価行: 対象時刻 T と起点 s の両方がその期間内（各期間の先頭 h 時間は除外）", { text: "リーク防止", head: true }, "起点までの観測から作る特徴量は、未来のデータを切り落としても値が変わらないことをテストで確認", "標準化・早期終了・残差／直接の選択は検証期間のみ。テスト期間の結果でモデルを再選択しない（EDA は全期間）"], M, 2.85, 6.4, 3.7, { size: 13 });
...
// ============================================================ 7 精度
{
  const s = base();
  kicker(s, "結果 1 — 精度");
  message(s, "6 時間先の MAE は設備 2 で 4.43（持続）→ 2.37（リッジ）→ 1.95 ℃（LightGBM）。設備 1 は 1.15 → 1.11 ℃で、改善は小さい。");
  ...
  table(s, [
    ...
    ["設備 2（ETTh2）", "0.91 / 0.50 / 0.37", { text: "4.43 / 2.37 / 1.95", bold: true }, "3.19 / 3.26 / 3.17"],
...
// ============================================================ 8 安定性
{
  ...
  bullets(s, ["設備 2: 週単位で 97% の週で持続に、91% の週でリッジに勝つ", "設備 1: 月によって負ける（2018-01 は持続 0.94 ℃に対し 1.26 ℃）。3.6% の改善を安定した効果とは言わない", "時刻別: 午前の上昇局面で持続予測の誤差が最も大きく、モデルとの差もそこで開く"], M, 5.25, W - 2 * M, 1.3, { size: 12.5, gap: 4 });
}
...
// ============================================================ 10 事前検知
{
  ...
  ], M, 4.85, 6.6, [2.4, 1.4, 1.4, 1.4], { size: 10.5, rowH: 0.32 });
  box(s, M + 7.0, 1.85, W - 2 * M - 7.0, 4.7, "F0F1F4");
  label(s, "定義と注意", M + 7.25, 2.0, 4.8);
  bullets(s, ["対象は「起点では閾値以下、対象時刻で超える」区間。連続した実測超過は 1 区間、連続した事前警報は 1 通知に集約", "先行時間は、検知した各区間について「最初の的中予測を出した起点」から「実際の超過開始」までを測る。集約した通知の開始時刻からではない", "的中した通知 9 件と検知した区間 10 件は別の集計単位（1 通知が複数の区間に対応しうる）。誤通知は、通知の間に一度も超えなかったもの", "区間は 2018 年 5〜6 月に集中し、同日の再超過を含む", "設備 1 はテスト期間に超過 0 件のため評価保留。上位 10% 点での補助評価は付録 B"], M + 7.25, 2.4, 4.85, 4.1, { size: 10.5, gap: 4 });
}
...
// ============================================================ 12 判断と限界
{
  ...
  bullets(s, ["24 時間先は、今回比較したモデル・特徴量では持続予測を明確に上回る改善を確認できなかった。外気温・気象予報の追加は次に検証する候補", "設備 2 台のみ。未学習設備への汎化は未検証", "高温区間が 5〜6 月に集中し、閾値は相対値。危険温度での検知は未評価", "故障・点検・介入の記録が無く、保全効果は評価できない", "短い高温区間を過小予測した例がある（11 枚目）"], M + 6.43, 2.4, 5.5, 4.0, { size: 13, gap: 9 });
}
