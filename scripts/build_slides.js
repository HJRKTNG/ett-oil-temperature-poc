 ...
 * 罫線・見出し行の塗り・解釈の項目ごとの細い区切り線と、要点の項目だけ紺の太字にする見せ方は、Codex の案を本人が採用した（2026-09-18）。
...
function title(s, text, { color = NAVY, size = 24, y = 0.34, h = 0.95 } = {}) {
...
function appendixLabel(s, text) {
  s.addText(text, { x: M, y: 0.14, w: CW, h: 0.26, fontFace: FONT, fontSize: 11, bold: true, color: MUTED, margin: 0, isTextBox: true });
}
// 項目の中の "\n" は、同じ箇条書きの中での改行（語の途中で折り返さないよう、改行位置を自分で決める）
function bullets(s, items, { x = M, y = 1.5, w = CW, h = 3.3, size = 20, gap = 12, color = BODY, emphasis = [0] } = {}) {
...
// 縦線を使わず、見出し行のグレーと罫線で区切る表。rows[0] を見出し行にするかは head で指定。
function table(s, rows, { x = M, y = 1.5, w = CW, colW, size = 16, head = true, rowH = 0.5, bold = [] } = {}) {
  const data = rows.map((r, ri) => r.map((c, ci) => {
    ...
    return {
      text: String(c),
      options: {
        fontFace: FONT, fontSize: isHead ? Math.max(12, size - 2) : Math.max(14, size), color: isHead ? NAVY : (bold.includes(ci) ? NAVY : BODY), bold: isHead || bold.includes(ci),
        align: "left", valign: "middle", margin: [7, 8, 7, 6],
        fill: isHead ? "F2F2F2" : WHITE,
...
// ========== 6. 評価の設計
{
  ...
  const segs = [["学習　12か月", "2016年7月〜2017年6月", 12, NAVY], ["検証　4か月", "2017年7月〜10月", 4, MUTED], ["テスト　約8か月", "2017年11月〜2018年6月", 7.9, BLUE]];
  let x = x0;
...
// ========== 7. 結果1 精度
{
  ...
  text(s, "リッジ比", xL - 0.55, yTop - 0.98, 1.1, 0.24, { size: 11, color: BLUE, align: "center" });
  text(s, `−${cut(r6.ridge, r6.lgbm)}%`, xL - 0.55, yTop - 0.73, 1.1, 0.34, { size: 18, color: BLUE, bold: true, align: "center" });
  const r6b = raw.ETTh1["6"], r1 = mae.ETTh2["1"];
  bullets(s, [
    `設備1は、最良の比較手法より\n${cut(r6b.persistence, r6b.lgbm)}%小さいだけ`,
    `設備2の1時間先は、\n持続予測の${f2(r1.persistence)}℃に対して\n${f2(r1.lgbm)}℃`,
    "24時間先は明確な改善なし\n（付録B）",
  ], { x: 6.25, y: 1.5, w: 3.25, h: 3.3, size: 16, gap: 14, emphasis: [] });
...
// ========== 10. 結果4 高温の事前検知
{
  ...
  bullets(s, [
    ...
    "高温区間は5〜6月に集中",
  ], { x: 6.25, y: 1.5, w: 3.25, h: 3.3, size: 16, gap: 14, emphasis: [0, 1] });
...
}
{
  ...
  bullets(s, [
    ...
    `${issued}時に警報を出した`,
    `${a}〜${first - 1}時の超過は\n予測できなかった`,
  ], { x: 6.9, y: 1.5, w: 2.6, h: 3.3, size: 15, gap: 12, emphasis: [1, 2] });
...
// ========== 12. 設計上の判断
{
  ...
  table(s, [
    ...
    ["1・6・24時間先で別々のモデル", "予測値の再入力による\n誤差の蓄積を避ける"],
    ["絶対誤差で学習", "評価指標のMAEに合わせる"],
    ["特徴量と閾値はテスト前に固定", "テスト結果を見た選び直しを防ぐ"],
  ], { y: 1.5, colW: [4.7, 4.3], size: 16, rowH: 0.62, bold: [0] });
}
...
// ========== 14. 結論と次の一手
{
  ...
  text(s, `設備2は6時間先の誤差を${cut(r6.ridge, r6.lgbm)}%減らし、\n高温${lg.events}区間中${lg.detected}区間を事前に検知`, M, 1.65, CW, 0.75, { size: 22, color: WHITE, bold: true });
  text(s, "保全効果と、評価していない設備での有効性は未検証", M, 2.55, CW, 0.38, { size: 18, color: TINT });
  hline(s, M, 3.12, CW, TINT, 0.75);
  text(s, "試行では、通知候補の数・先行時間・見逃しを記録する\n許容できる誤通知の数は、現場と決める", M, 3.4, CW, 0.75, { size: 18, color: WHITE });
...
// ========== 付録A 特徴量とモデル設定
{
  ...
  ], { y: 1.55, colW: [1.8, 7.2], size: 14, rowH: 0.7, head: false, bold: [0] });
}
...
// ========== 付録B 全結果
{
  ...
  const cell = (t, o = {}) => ({ text: t, options: { fontFace: FONT, fontSize: 14, color: BODY, align: "center", valign: "middle", margin: [5, 2, 5, 2], border: [none, none, line, none], ...o } });
  const head = (t, o = {}) => cell(t, { fontSize: 11, color: NAVY, fill: "F2F2F2", ...o });
  const rows = [
  ...
  bullets(s, [
    `設備2の1時間先は、\n${f2(mae.ETTh2["1"].persistence)}℃から${f2(mae.ETTh2["1"].lgbm)}℃へ\n（−${cut(r1.persistence, r1.lgbm)}%）`,
    "24時間先のLightGBMは\n持続予測に比べて、\n設備1で悪化、\n設備2でほぼ同じ",
...
// ========== 付録C 見逃した例
{
  ...
  bullets(s, [
    ...
    "次の検証で、\n通知の閾値を下げたときの\n見逃しと誤通知を確かめる",
  ], { x: 6.9, y: 1.55, w: 2.6, h: 3.3, size: 14, gap: 10, emphasis: [1] });
